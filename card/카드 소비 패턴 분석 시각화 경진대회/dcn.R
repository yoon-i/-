...
geom_text(aes(label = paste(prop*100, "%")), size = 5, position = position_stack(.5))


gender <- round(table(data1$성별)/sum(table(data1$성별)) *100, 1)

round(table(data6_2$계절)[1:4]/table(data6_2$소분류)*100,1)
...
data6$계절 <- 
  if(data6$기준년월 == 201601|201602|201612|201701|201702|201712|201801|201802|201812|201901|201902|201912|202001|202002|202012|202101|202102){
    '겨울'
  } else if(data6$기준년월 == 201603|201604|201605|201703|201704|201705|201803|201804|201805|201903|201904|201905|202003|202004|202005|202103|202104|202105){
    '봄'
  } else if(data6$기준년월 == 201606|201607|201608|201706|201707|201708|201806|201807|201808|201906|201907|201908|202006|202007|202008|202106|202107){
    '여름'
  } else {
    '가을'
  }
...
if(data6$기준년월 == c(201601,201602,201612,201701,201702,201712,201801,201802,201812,201901,201902,201912,202001,202002,202012,202101,202102)){
  '겨울'
} else if(data6$기준년월 == c(2201603,201604,201605,201703,201704,201705,201803,201804,201805,201903,201904,201905,202003,202004,202005,202103,202104,202105)){
  '봄'
} else if(data6$기준년월 == c(201606,201607,201608,201706,201707,201708,201806,201807,201808,201906,201907,201908,202006,202007,202008,202106,202107)){
  '여름'
} else {
  '가을'
}

data6$계절 <- if(data6$기준년월 == 201601){'겨울'}


data6$계절 <- switch(data6$기준년월,
                   201601 == '겨울',
                   '봄')



data6$계절 <- if(str_sub(data6$기준년월, start=5, end=6) == '01') '겨울'

data6$계절 <-'겨울'


str_sub(data6$기준년월, start=5, end=6) == '01'
str_sub(data6$기준년월, start=5, end=6) == '02'
str_sub(data6$기준년월, start=5, end=6) == '03'
str_sub(data6$기준년월, start=5, end=6) == '04'
str_sub(data6$기준년월, start=5, end=6) == '05'
str_sub(data6$기준년월, start=5, end=6) == '06'
str_sub(data6$기준년월, start=5, end=6) == '07'
str_sub(data6$기준년월, start=5, end=6) == '08'
str_sub(data6$기준년월, start=5, end=6) == '09'
str_sub(data6$기준년월, start=5, end=6) == '10'
str_sub(data6$기준년월, start=5, end=6) == '11'
str_sub(data6$기준년월, start=5, end=6) == '12'


data6$월 <- str_sub(data6$기준년월,start = 5, end = 6)
data6$월 <- as.integer(data6$월)

data6[as.integer(str_sub(data6$기준년월, start=5, end=6)) <= 3, '절']='겨울'

date6$계절 <- if(data6$월>=3){
  '봄'
}else if(data6$월>=6){
  '여름'
}else if(data6$월>=9){
...
data6$계절 <- ifelse(data6$월 < 3|data6$월 ==12, "겨울", "봄")
data6$계절 <- if(data6$월 >= 6) 여름


data6$계절 <- if(data6$계절<=03){'겨울'}

data6$절 <- 
  
  subset(Sales, 가격 ==200)

subset(data6, 기준년월 == 201603)


data6$계절 <- '겨울'
data6 %>% filter(기준년월 == c(201601,201602,201612,201701,201702,201712,201801,201802,201812,201901,201902,201912,202001,202002,202012,202101,202102))

data6$계절 <- ifelse(data6$기준년월== c(201601,201602,201612,201701,201702,201712,201801,201802,201812,201901,201902,201912,202001,202002,202012,202101,202102), '겨울','여름')

# 기준년월을 날짜형식으로
str(data6$기준년월) # int기 때문에 문자로 변환 후 날짜형식으로 변환
data6$기준년월 <- as.character(data6$기준년월)
data6$기준년월 <- as.Date(data6$기준년월, format='%Y%m')
str(data6$월)
...
if(data3_1$가맹점주소광역시도=='서울' & str_sub(data3_1$가맹점주소시군구,3,3) == '시') data3_1$가맹점주소광역시도<-'경기'


a <- data3_1$가맹점주소광역시도=='서울' & str_sub(data3_1$가맹점주소시군구,3,3) == '시'


data3_1$가맹점주소광역시도 <- 경기

test <- data3_1
test$가맹점주소광역시도 <- ifelse(test$가맹점주소시군구 == '강남구', '서울', '')
...
ggplot(gn, aes(업종대분류, 카드이용금액계)) +
  geom_col()


ggplot(data3_1, aes(reorder(가맹점주소시군구,-카드이용건수계),카드이용건수계)) +
  geom_col() +
  theme(axis.text.x=element_text(angle=90))
...
# 가맹점주소광역시도 비율 테이블
sido <- round(sort(table(data3$가맹점주소광역시도), decreasing = T)/sum(sort(table(data3$가맹점주소광역시도), decreasing = T))*100,1)
...
ggplot(data3, aes(가맹점주소광역시도,카드이용금액계, fill=가맹점주소광역시도)) +
  geom_col() +
  scale_y_continuous(labels = scales::comma) +
  ggtitle('시도별 카드이용금액계') +
  theme(legend.position='none')
# => 서울이 압도적으로 높고, 경기, 인천, 강원 순으로 높다.

ggplot(data3, aes(가맹점주소광역시도,카드이용건수계, fill=가맹점주소광역시도)) +
  geom_col() +
  scale_y_continuous(labels = scales::comma) +
  ggtitle('시도별 카드이용건수계') +
  theme(legend.position='none')
